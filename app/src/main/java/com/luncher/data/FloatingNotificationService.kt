...
class FloatingNotificationService : Service() {
    ...
    private fun disableKeyboard() {
        try {
            ...
            val imm = getSystemService(Context.INPUT_METHOD_SERVICE) as InputMethodManager
            floatingView?.let { imm.hideSoftInputFromWindow(it.windowToken, 0) }
        } catch (_: Exception) {}
    }
    ...
    fun refreshAggressive() {
        ...
        cont.removeAllViews()
        if (NotificationRepository.notifs.isEmpty()) { view.visibility = View.GONE; disableKeyboard(); return }
        ...
        for (notif in NotificationRepository.notifs.toList()) {
            ...
            addBtn("FERMER", Color.parseColor("#FF0000")) {
            ...
            notif.notification.actions?.forEach { action ->
                ...
                val col = when {
                    isReply -> Color.BLACK
                    actionTitle.contains("chiv", true) -> Color.parseColor("#FF666666")
                    else -> Color.parseColor("#FF1565C0")
                }
                addBtn(actionTitle.uppercase(), col) {
                    if (isReply && action.remoteInputs != null && action.remoteInputs.isNotEmpty()) {
                        ...
                        val edit = EditText(ctx).apply {
                            ...
                            setBackgroundResource(R.drawable.white_rounded_bg)
                        ...
                        sendBtn.setOnClickListener {
                            ...
                            try {
                                ...
                                Toast.makeText(this, "Reponse envoyee", Toast.LENGTH_SHORT).show()
                            } catch (e: Exception) {
            ...
            if (notif.notification.actions.isNullOrEmpty()) {
                addBtn("OUVRIR", Color.parseColor("#FF1565C0")) {
                    try {
