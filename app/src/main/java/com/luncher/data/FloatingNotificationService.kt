...
class FloatingNotificationService : Service() {
    ...
    fun refreshAggressive() {
        ...
        for (notif in NotificationRepository.notifs.toList()) {
            ...
            addBtn("FERMER", Color.parseColor("#FF0000")) { NotificationRepository.notifs.remove(notif); try { NotificationListener.getInstance()?.cancelNotification(notif.sbnKey) } catch (_: Exception) {}; refreshAggressive() }
            notif.notification.actions?.forEach { action -> val actionTitle = action.title.toString(); val isReply = action.remoteInputs != null && action.remoteInputs.isNotEmpty(); val col = if (isReply) Color.BLACK else Color.parseColor("#FF1565C0"); addBtn(actionTitle.uppercase(), col) { if (isReply) { actionsContainer.removeAllViews(); val inputLayout = LinearLayout(ctx).apply { orientation = LinearLayout.HORIZONTAL; setPadding(0,8,0,0) }; val edit = EditText(ctx).apply { hint = "Répondre..."; setHintTextColor(Color.parseColor("#FF999999")); setTextColor(Color.BLACK); setBackgroundResource(R.drawable.white_rounded_bg); layoutParams = LinearLayout.LayoutParams(0, ViewGroup.LayoutParams.WRAP_CONTENT, 1f).apply { marginEnd = 12 }; setPadding(20,16,20,16) }; val sendBtn = TextView(ctx).apply { text = "ENVOYER"; setTextColor(Color.BLACK); textSize = 13f; setPadding(24,16,24,16); isClickable = true }; sendBtn.setOnClickListener { val replyText = edit.text.toString(); if (replyText.isBlank()) return@setOnClickListener; try { val remoteInputs = action.remoteInputs; val results = android.os.Bundle(); for (ri in remoteInputs) { results.putCharSequence(ri.resultKey, replyText) }; val fillIntent = Intent(); android.app.RemoteInput.addResultsToIntent(remoteInputs, fillIntent, results); action.actionIntent.send(this, 0, fillIntent); Toast.makeText(this, "Réponse envoyée", Toast.LENGTH_SHORT).show() } catch (_: Exception) {}; disableKeyboard(); NotificationRepository.notifs.remove(notif); try { NotificationListener.getInstance()?.cancelNotification(notif.sbnKey) } catch (_: Exception) {}; refreshAggressive() }; inputLayout.addView(edit); inputLayout.addView(sendBtn); actionsContainer.addView(inputLayout); edit.post { enableKeyboard(edit) } } else { try { action.actionIntent.send() } catch (_: Exception) {}; NotificationRepository.notifs.remove(notif); refreshAggressive() } } }
            ...
            if (directReplyAction != null) { addBtn("REPONDRE", Color.BLACK) { actionsContainer.removeAllViews(); val inputLayout = LinearLayout(ctx).apply { orientation = LinearLayout.HORIZONTAL; setPadding(0,8,0,0) }; val edit = EditText(ctx).apply { hint = "Répondre..."; setHintTextColor(Color.parseColor("#FF999999")); setTextColor(Color.BLACK); setBackgroundResource(R.drawable.white_rounded_bg); layoutParams = LinearLayout.LayoutParams(0, ViewGroup.LayoutParams.WRAP_CONTENT, 1f).apply { marginEnd = 12 }; setPadding(24,20,24,20) }; val sendBtn = TextView(ctx).apply { text = "ENVOYER"; setTextColor(Color.BLACK); textSize = 14f; setPadding(24,16,24,16); isClickable = true }; sendBtn.setOnClickListener { val replyText = edit.text.toString(); if (replyText.isBlank()) return@setOnClickListener; try { val remoteInputs = directReplyAction.remoteInputs; val results = android.os.Bundle(); for (ri in remoteInputs) { results.putCharSequence(ri.resultKey, replyText) }; val fillIntent = Intent(); android.app.RemoteInput.addResultsToIntent(remoteInputs, fillIntent, results); directReplyAction.actionIntent.send(this, 0, fillIntent); Toast.makeText(this, "Réponse envoyée", Toast.LENGTH_SHORT).show() } catch (_: Exception) {}; disableKeyboard(); NotificationRepository.notifs.remove(notif); try { NotificationListener.getInstance()?.cancelNotification(notif.sbnKey) } catch (_: Exception) {}; refreshAggressive() }; inputLayout.addView(edit); inputLayout.addView(sendBtn); actionsContainer.addView(inputLayout); edit.post { enableKeyboard(edit) } } }
            cont.addView(card)
        }
    }
