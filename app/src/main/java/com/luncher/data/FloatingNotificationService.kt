...
class FloatingNotificationService : Service() {
    ...
    fun refreshAggressive() {
        ...
        for (notif in NotificationRepository.notifs.toList()) {
            ...
            notif.notification.actions?.forEach { action -> val t = action.title.toString(); val isReply = action.remoteInputs != null && action.remoteInputs.isNotEmpty(); if (!isReply) { addBtn(t.uppercase(), Color.parseColor("#FF1565C0")) { try { action.actionIntent.send() } catch (_: Exception) {}; NotificationRepository.notifs.remove(notif); refreshAggressive() } } }
