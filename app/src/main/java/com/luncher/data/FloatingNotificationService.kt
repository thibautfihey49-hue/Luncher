...
class FloatingNotificationService : Service() {
    ...
    fun refreshAggressive() {
        ...
        for (notif in NotificationRepository.notifs.toList()) {
            ...
            notif.notification.actions?.forEach { action -> val actionTitle = action.title.toString(); val isReply = action.remoteInputs != null && action.remoteInputs.isNotEmpty(); if (!isReply) { val col = Color.parseColor("#FF1565C0"); addBtn(actionTitle.uppercase(), col) { try { action.actionIntent.send() } catch (_: Exception) {}; NotificationRepository.notifs.remove(notif); refreshAggressive() } } }
            ...
            addBtn("REPONDRE", Color.BLACK) {
                ...
                val edit = EditText(ctx).apply { hint = "Répondre..."; setHintTextColor(Color.parseColor("#FF999999")); setTextColor(Color.BLACK); setBackgroundResource(R.drawable.white_rounded_bg); layoutParams = LinearLayout.LayoutParams(0, ViewGroup.LayoutParams.WRAP_CONTENT, 1f).apply { marginEnd = 12 }; setPadding(24,20,24,20) }
