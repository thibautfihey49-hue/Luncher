...
class FloatingNotificationService : Service() {
    ...
    fun refreshAggressive(){
        ...
        for(notif in NotificationRepository.notifs.toList()){
            ...
            card.findViewById<TextView>(R.id.notifTitle).text="actions=${notif.notification.actions?.size ?: 0}"
            ...
            btn("FERMER", Color.RED){ NotificationRepository.notifs.remove(notif); try{NotificationListener.getInstance()?.cancelNotification(notif.sbnKey)}catch(_:Exception){}; refreshAggressive() }
