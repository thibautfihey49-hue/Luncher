...
class SmsAppActivity : AppCompatActivity() {
    ...
    inner class MsgAdapter(val items:List<Msg>): RecyclerView.Adapter<RecyclerView.ViewHolder>(){
        ...
        override fun onCreateViewHolder(p:android.view.ViewGroup, vt:Int): RecyclerView.ViewHolder{
            return when(vt){
                1->{ val ll=android.widget.LinearLayout(this@SmsAppActivity).apply{ orientation=android.widget.LinearLayout.VERTICAL; setPadding(12,8,12,8) }; val img=ImageView(this@SmsAppActivity).apply{ id=R.id.img; layoutParams=android.widget.LinearLayout.LayoutParams(600,600) }; val txt=TextView(this@SmsAppActivity).apply{ id=R.id.txt }; ll.addView(img); ll.addView(txt); object: RecyclerView.ViewHolder(ll){} }
                2->{ val ll=android.widget.LinearLayout(this@SmsAppActivity).apply{ orientation=android.widget.LinearLayout.HORIZONTAL; setPadding(16,12,16,12) }; val btn=TextView(this@SmsAppActivity).apply{ id=R.id.play; text="▶️" }; ll.addView(btn); object: RecyclerView.ViewHolder(ll){} }
