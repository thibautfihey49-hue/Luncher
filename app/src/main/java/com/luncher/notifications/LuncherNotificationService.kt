...
class LuncherNotificationService: NotificationListenerService(){
    override fun onNotificationPosted(sbn: StatusBarNotification?){
        ...
        if(pkg!in listOf("com.whatsapp","com.whatsapp.w4b","com.google.android.gm","com.android.mms","com.google.android.apps.messaging","com.facebook.orca","com.instagram.android")) return
        ...
        val notif=LuncherNotif(sbn.key, pkg, title, text, sbn.postTime, actions, sbn)
    ...
    companion object{
        fun sendQuickReply(notif: LuncherNotif, replyText:String){
            try{
                // Cherche l'action avec RemoteInput
                for(action in notif.actions){
                    val remoteInputs=action.remoteInputs
                    if(remoteInputs!=null && remoteInputs.isNotEmpty()){
                        val intent=Intent()
                        val bundle=Bundle()
                        bundle.putString(remoteInputs[0].resultKey, replyText)
                        RemoteInput.addResultsToIntent(remoteInputs, intent, bundle)
                        action.actionIntent.send(notif.statusBar.context, 0, intent)
                        return
                    }
                }
                // Fallback pour WhatsApp style - wearable extender
                val wearable = androidx.core.app.NotificationCompat.WearableExtender(notif.statusBar.notification)
