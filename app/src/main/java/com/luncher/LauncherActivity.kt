...
class LauncherActivity : AppCompatActivity() {
    ...
    private fun filterApps(q: String){
        filteredApps = if(q.isBlank()) allApps else allApps.filter { it.label.contains(q, true) || it.packageName.contains(q, true) }
        appAdapter.update(filteredApps)
    }

    private fun launchApp(app: AppInfo){
        try{
            val i = packageManager.getLaunchIntentForPackage(app.packageName)?: Intent().apply{
