...
class LauncherActivity : AppCompatActivity() {
    ...
    private val pickImageLauncher = registerForActivityResult(ActivityResultContracts.GetContent()) { uri: Uri? -> if (uri != null) { try { contentResolver.takePersistableUriPermission(uri, Intent.FLAG_GRANT_READ_URI_PERMISSION) } catch (_: Exception) {}; prefs.edit().putString("wallpaper_uri", uri.toString()).apply(); loadWallpaper() } }
    ...
    private fun loadWallpaper() {
        ...
        if (uriStr != null) {
            try { wallpaperView.setImageURI(Uri.parse(uriStr)); wallpaperView.setBackgroundColor(Color.TRANSPARENT); return } catch (_: Exception) {}
        }
    ...
    private fun setupWallpaperLongPress() {
        val l = View.OnLongClickListener { if (drawerContainer.visibility != View.VISIBLE) pickImageLauncher.launch("image/*"); true }
        root.setOnLongClickListener(l); wallpaperView.setOnLongClickListener(l)
    }
    private fun setupTimeDragging() {
        ...
        timeContainer.setOnLongClickListener { view -> view.setTag(R.id.drawerHandle, true); view.performHapticFeedback(android.view.HapticFeedbackConstants.LONG_PRESS); Toast.makeText(this, "Deplace l'heure", Toast.LENGTH_SHORT).show(); true }
    }
    ...
    private fun setupDrawer() {
        ...
        drawerHandle.setOnTouchListener { _, event ->
            ...
            if (event.action == MotionEvent.ACTION_UP) { if (startY - event.rawY > 100) openDrawer() }
            true
        }
        root.setOnTouchListener { _, event ->
            if (drawerContainer.visibility == View.VISIBLE) false else {
                if (event.action == MotionEvent.ACTION_DOWN) startY = event.rawY
                if (event.action == MotionEvent.ACTION_UP) { val diff = startY - event.rawY; val fromBottom = root.height - event.rawY < 200; if (diff > 120 && fromBottom) openDrawer() }
                false
            }
        }
        var searchStartY = 0f
        drawerSearchHandle.setOnTouchListener { _, event ->
            ...
            if (event.action == MotionEvent.ACTION_UP) { if (event.rawY - searchStartY > 100) closeDrawer() }
            true
        }
        ...
        searchBar.setOnTouchListener { _, event ->
            ...
            if (event.action == MotionEvent.ACTION_DOWN) searchStartY = event.rawY
            if (event.action == MotionEvent.ACTION_UP && event.rawY - searchStartY > 120) { closeDrawer(); return true }
            false
        }
    }
    ...
    private fun closeDrawer() { drawerContainer.animate().alpha(0f).setDuration(180).withEndAction { drawerContainer.visibility = View.GONE; searchBar.setText("") }.start(); val imm = getSystemService(Context.INPUT_METHOD_SERVICE) as android.view.inputmethod.InputMethodManager; currentFocus?.let { imm.hideSoftInputFromWindow(it.windowToken, 0) } }
    private fun setupRecycler() {
        appRecycler.layoutManager = GridLayoutManager(this, 4)
        adapter = AppAdapter(this, emptyList()) { app ->
            try { val intent = Intent().apply { component = android.content.ComponentName(app.packageName, app.className); addFlags(Intent.FLAG_ACTIVITY_NEW_TASK) }; startActivity(intent); closeDrawer() } catch (_: Exception) { try { val launch = packageManager.getLaunchIntentForPackage(app.packageName); if (launch != null) { startActivity(launch); closeDrawer() } } catch (_: Exception) {} }
        }
