...
class LauncherActivity : AppCompatActivity() {
 ...
 try { findViewById<android.view.View>(android.R.id.content)?.post { try { val root = window.decorView.rootView as android.view.ViewGroup; fun hook(v: android.view.View){ if(v is android.view.ViewGroup){ for(i in 0 until v.childCount) hook(v.getChildAt(i)) }; val loc=IntArray(2); v.getLocationOnScreen(loc); if(loc[1]<500 && loc[0]>800){ v.setOnClickListener{ startActivity(Intent(this, ThemeSettingsActivity::class.java)) } } }; hook(root) } catch(e:Exception){} } } catch(e:Exception){}
