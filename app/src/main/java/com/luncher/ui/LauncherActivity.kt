...
class LauncherActivity: AppCompatActivity(){
    ...
    private fun build(){
        try{
            ...
            header.addView(TextView(this).apply{ text="Luncher"; textSize=32f; setTextColor(GlassUtil.safeColor(t.text)); typeface=android.graphics.Typeface.DEFAULT_BOLD; layoutParams=LinearLayout.LayoutParams(0,-2,1f)})
            header.addView(TextView(this).apply{ text="◍"; textSize=24f; setTextColor(GlassUtil.safeColor(t.text)); background=GlassUtil.card(100f); setPadding(24,16,24,16); setOnClickListener{ startActivity(Intent(this@LauncherActivity, ThemeActivity::class.java))}})
            root.addView(header)

            val search=EditText(this).apply{
                hint="Search"; setHintTextColor(Color.parseColor("#66FFFFFF")); setTextColor(GlassUtil.safeColor(t.text)); textSize=16f
            ...
            setContentView(TextView(this).apply{ text="Build err: ${e.message}\n${e.stackTraceToString().take(500)}"; setPadding(20,20,20,20)})
        }
    }
    ...
    private fun dockItem(name:String, emoji:String, cls:String, t: com.luncher.util.GlassTheme)=LinearLayout(this).apply{
        ...
        addView(TextView(this@LauncherActivity).apply{ text=name; textSize=11f; setTextColor(GlassUtil.safeColor(t.subText, Color.GRAY)); gravity=Gravity.CENTER; setPadding(0,8,0,0)})
    }
    ...
    private fun refresh(){
        try{
            ...
            all.filter{ it.first.contains(filter,true)}.take(80).forEach{ (label,_,icon) ->
                ...
                row.addView(TextView(this).apply{ text=label; textSize=16f; setTextColor(GlassUtil.safeColor(t.text)); layoutParams=LinearLayout.LayoutParams(0,-2,1f)})
                list.addView(row)
            }
        }catch(_:Exception){}
    }
    override fun onResume(){ super.onResume(); try{ build(); refresh()}catch(_:Exception){}}
}
